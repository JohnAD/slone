...
HANDLING NONE VS NULL VS EMPTY
==============================

Most computer languages do not have a means of identifying "unknown" (null) vs
none.

One of the reasons that SLONE does not support "nothing" for a value is to
make the distinctions less important since now each element only allows one
of those: "name" and "type" support nothing (``_``) but not null (``?``).
"value" supports null but not nothing.

Pretty much all languages support the concept of "empty". For example, an
empty array ([]) or an empty integer (zero).

.. code:: python

    # "a" is an empty list, "b" does not exist, "c" is an unknown list
    x["a"] = []
    del x["b"]
    x["c"] = None  # we will treat None to mean Null/unknown despite the name
